...
function actionLabel(action: QueueAction) {
  return { contacted: "Carrier contact simulated", response: "Carrier response logged", eta: "ETA review simulated — source unchanged", customer: "Customer update simulated — not sent", snooze: "Snoozed until demo reset", escalate: "Escalated to carrier relations", reassign: "Reassigned to Jordan Brooks", resolved: "Marked resolved" }[action];
}

export function MyQueue() {
  const shared = useDemoSession();
  const { exceptions, shipments } = shared;
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const statuses = shared.session.statuses;
  const owners = shared.session.owners;
  const lastActions = shared.session.actions;
  const [toast, setToast] = useState<string | null>(null);
  const [search, setSearch] = useState("");
  const [now] = useState(() => DEMO_NOW);
  ...
  const update = (action: QueueAction) => {
    ...
    dispatchDemo({ type: "exception", id, status: action === "resolved" ? "resolved" : action === "snooze" ? "snoozed" : "in_progress", owner: action === "escalate" ? "Carrier Relations" : action === "reassign" ? "Jordan Brooks" : undefined, label: actionLabel(action) });
    if (action === "customer" && selected.shipment) dispatchDemo({ type: "communication", shipmentId: selected.shipment.id, text: "Customer update preview logged from My Queue" });
  ...
  return (
    <div className="min-h-screen bg-[#0b0e12] text-slate-100">
      <aside className={`fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-white/10 bg-[#101419] transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="space-y-1 px-3 py-5">{navItems.map(([label, Icon, href]) => <Link key={label} href={href} className={`flex items-center gap-3 rounded px-3 py-2.5 text-sm ${label === "My Queue" ? "bg-cyan-400/10 text-cyan-300" : "text-slate-400 hover:bg-white/5 hover:text-slate-100"}`}><Icon size={17} strokeWidth={1.7} /><span>{label}</span>{label === "My Queue" && <span className="ml-auto rounded-full bg-cyan-400/15 px-2 py-0.5 text-[10px] text-cyan-300">{ranked.length}</span>}</Link>)}</nav>
